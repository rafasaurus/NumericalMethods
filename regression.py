...
#computes matrix inverse using Fadeev algorithm
...
#-------program-------
n = 4
k = 3 #the number of regression features --- ազատության աստիճան
k += 1

featureSize = 10
featureRows = 3

muffin = np.zeros(shape=(featureRows, featureSize))

arr = np.zeros(shape=(4, 4))
rate = np.array([0, 1, 2, 3, 4, 5], np.float)
muffin[0] = np.array([40,    130,     55,  65,   70,  43,   35,   110,   89,     78], np.float)#square meters
muffin[1] = np.array([1000, 13000, 6500, 8000, 11000,12000, 7000, 17000, 8900,  5000], np.float)#house price
muffin[2] = np.array([2,     5,      6,    3,    2,    5 ,    6,    8,    8,     9],np.float)#house gread scale 1-10 for distance to metro

yp = np.array([44,    110,     78,  75,   40,  45,   37,   134,   90,     90], np.float)


arr = COMPUTE_REGRESSION_X(muffin, featureRows, featureSize, featureRows+1)
print(arr)
inverse_arr = INVERSE_MATRIX(arr,featureRows+1)
print(inverse_arr)

#vector = COMPUTE_REGRESSION_Y_EX(yp, muffin[0], muffin[1], featureSize)
#print("arr=", arr)
#inverse_arr = INVERSE_MATRIX(arr, 4)
#print("vector=\n", vector)
