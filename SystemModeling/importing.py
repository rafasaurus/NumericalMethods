...
def include(N,df):
    ...
    for column in df:
        ...
        if column != max_corr_string and column!='Y' and iter_q_iter!=0:
            ...
            max_j=0
            max = .0
            for j in range(3):#(len(R_df_copy.columns)):
                if R_SQUARED_CYCLE == True:
                    R_SQUARED_CYCLE = False
                    max = ((R_df_corr_inverse[0][j])**2) / (R_df_corr_inverse[0][0]*R_df_corr_inverse[j][j])
                    max_j=j

                else:
                    middle = ((R_df_corr_inverse[0][j])**2) / (R_df_corr_inverse[0][0]*R_df_corr_inverse[j][j])
                    if (middle > max):
                        max = ((R_df_corr_inverse[0][j])**2) / (R_df_corr_inverse[0][0]*R_df_corr_inverse[j][j])
                        max_j = j
                        print("max=",max)
